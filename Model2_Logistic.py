...
The features are selected by the variables used in the class
Input Parameters: 5
    1. ADT 2. PTRUCK 3. Age 4. Age Square 5. Reconstruction Parameter
Output Parameter: 1
...
features = ['ADT_029','PERCENT_ADT_TRUCK_109','recons','SVCYR','SVCYR_sq'] # 5 input parameters
X = a[features] # Dataframe of input features
Y = a['culv_cond'] # Dataframe of output
...
# For undersampling of the majority data the following code could be used

"""
over = SMOTE(sampling_strategy=0.3) # fraction of majority data for the minor data
under = RandomUnderSampler(sampling_strategy=0.3) # undesampling of the majority data
steps = [('o', over), ('u', under)] # creating the over & under sampled data 
pipeline = Pipeline(steps=steps) # creating the pipline
X, Y = pipeline.fit_resample(X, Y) # creating the new dataset
"""
...
# Initiating the model object using the regularization parameter c = 5

logreg = LogisticRegression(C = 5)

# Fitting the model with data in for logistic regression
...
y_pred = logreg.predict(X_test) # fitting the testing data
yprob = logreg.predict_proba(X_test) # output the probabilities
zz = pd.DataFrame(yprob) # converting into dataframe
#zz.to_csv('probs2.csv')
...
print("The coefficients of the model: ", logreg.coef_) #model coefficients
print("The intercept of the model: ", logreg.intercept_)#model intercept
"""
...
plt.title("Model 2 - Logistic Regression with variables used in class")
plt.grid()
plt.show()

# Create the confusion matrix and plotting it
...
make_confusion_matrix(cnf_matrix, group_names=labels,
                      ...
                      title="Model 2 - Logistic Regression with variables used in class")
